...
/* ─────────────────────────────────────────────────────────────
   Sub-item (Formulario)
───────────────────────────────────────────────────────────── */
const FormularioItem: React.FC<{ formulario: MiFormulario }> = ({ formulario }) => {
  const { theme } = useTheme();
  const c = theme.colors;
  const router   = useRouter();
  const pathname = usePathname();
  const isActive = pathname === formulario.ruta || pathname.startsWith(formulario.ruta + "/");
  ...
  return (
    <Animated.View style={animStyle}>
      ...
      >
        ...
        <View style={[styles.subLine, { backgroundColor: isActive ? c.primary : c.border }]} />
...
const ModuloItem: React.FC<{ modulo: MiModulo }> = ({ modulo }) => {
  ...
  const isActive = !hasChildren && (pathname === href || pathname.startsWith(href + "/"));
  ...
  return (
    <View style={[styles.sidebar, { backgroundColor: c.card, borderRightColor: c.border }]}>
      ...
      >
        ...
        {!loading && !error && modulos.map((modulo) => (
