...
export function EditECModal({
  ...
  useEffect(() => {
    if (ec) {
      setNombre(ec.nombre);
      setObservaciones(ec.observaciones ?? "");
    }
  }, [ec]);

  const handleConfirm = async () => {
    if (ec && nombre.trim()) {
      await onConfirm(ec.id, nombre.trim(), observaciones.trim() || undefined);
    }
  };
        ...
        >
          ...
          <View style={[styles.header, { borderBottomColor: c.border }]}>
            <Text style={[styles.title, { color: c.text }]}>
              Editar Elemento de Competencia
            </Text>
            <TouchableOpacity
              onPress={onClose}
              disabled={isLoading}
